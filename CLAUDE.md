...
# Backfill missing translated titles for existing raw_items
python scripts/backfill_item_titles.py --limit 100
...
- **OpenAI-compatible local endpoints**: Even when `AI_API_BASE` points to a local compatible server, LiteLLM/OpenAI clients may still require a non-empty API key. In local/dev setups, set `AI_API_KEY=dummy` if the upstream ignores the key.
